...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void playFootsteps() {
			if (speed > 0.85f && _footstepAudioCooldown <= 0f) {
				// pick random footstep audioClip to play
				int index = Random.Range (0, footstepsTileWalk.Length);
				AudioClip footstep = footstepsTileWalk[index];
				...
				if (_input.sprint) {
					GetComponent<AudioSource>().PlayOneShot(footstep, 1f);
					_footstepAudioCooldown = 0.4f;
				} else if (_input.tiptoe){
					GetComponent<AudioSource>().PlayOneShot(footstep, 0.1f);
